...
if (isset($vars['entity']) && !elgg_is_logged_in()) {
  ...
  if (elgg_is_sticky_form('AU_anonymous_comments')) {
	...
	if($speakfreelyformcounter != 1){  //first time, create the form..
		...
		$form_body .= "<p class='longtext_editarea'><label>".elgg_echo("generic_comments:text")."<br />" . elgg_view('input/longtext',array('name' => 'generic_comment', 'value' =>elgg_echo('AU_anonymous_comments:longtextwarning'))) . "</label></p>";
      ...
      if ($usessl == 'yes') {
		...
		$form_body .= elgg_view('input/submit', array('value' => elgg_echo("AU_anonymous_comments:post:comment"))) . "</p></div>";
